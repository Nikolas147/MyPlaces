...
//  MyPlaces
//
//  Created by Aleksey on 03/10/2019.
...
class MapManager {
    ...
    // Фокус карты на местоположения пользователя
    ...
    func getDirection(for mapView: MKMapView, previousLocation: (CLLocation) -> ()) {
        ...
        directions.calculate { (response, error) in
            ...
            for route in response.routes {
                ...
                print("Время в пути составит: \(timeInterval) час.")
                // добавить лэйблы, скрыть их при хагрузки и только тут их отобразить
                //если будет один, то типо норм
            }
        }
    }
    ...
    func createDirectionsRequest(from coordinate: CLLocationCoordinate2D) -> MKDirections.Request? {
        ...
        let requesr = MKDirections.Request()
        requesr.source = MKMapItem(placemark: startingLocation)
        requesr.destination = MKMapItem(placemark: destination)
        requesr.transportType = .automobile
        
        //отвечает за несколько вариантов пути
        requesr.requestsAlternateRoutes = true
...
//        }
    ...
    // Сбор всех ранее построенных маршрутов перед построением нового
